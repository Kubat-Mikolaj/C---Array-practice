...
The task is to write a C program that adds two square matrices of the same size. 
The program prompts the user to input the size of the matrices (less than 5), 
elements for each matrix, and then calculates the sum of the matrices. 
It displays the original matrices and their sum as output.
...
void Add_Array(int Array_a[MAX_NUMBER][MAX_NUMBER], int Array_b[MAX_NUMBER][MAX_NUMBER], int Array_c[MAX_NUMBER][MAX_NUMBER], int num);
...
int main()
{
    ...
    Add_Array(Array_a, Array_b, Array_c, MAX_NUMBER);
    printf("\nSum of two matrices: \n");
